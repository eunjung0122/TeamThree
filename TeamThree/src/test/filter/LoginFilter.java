 ...
 *	= => %3D
 *
 *	/users/loginform.jsp?url=%2Ftest%2Fxxx.jsp%3Fa%3Dxxx%b%3Dxxx
 *	인코딩 안하면 ?뒤가 다 짤려.. 
 */

/*
 * @WebFilter(urlPatterns={"필터링할 경로1","필터링할 경로2",...}) /String[]
 * 
 */
...
public class LoginFilter implements Filter{
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		//1.로그인을 했는지 로그인된 아이디를 읽어와본다.(HttpSession객체)
		HttpServletRequest req=(HttpServletRequest)request;
		HttpSession session=req.getSession();
		String id=(String)session.getAttribute("id");
		if(id!=null) {
			//2.만일 로그인을 했다면 관여하지 않고 요청의 흐름을 이어간다(FilterChain객체)
			...
			//3.로그인을 하지않았다면 /users/loginform.jsp 페이지로 리다일렉트 이동시킨다(HttpServletResponse 객체)
